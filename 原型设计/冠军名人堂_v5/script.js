        ...
        // This prevents background views from accumulating ~80 always-animating
        // DOM nodes and dragging down framerate (and hover/click responsiveness)
        // on whichever page the user is actually on.
        ...
        function showGame(key) {
            ...
            if (detailKey) {
                ...
                championBtn.onclick = (e) => curtainRevealToDetail(detailKey, e);
            } else {
        ...
        function showDetail(champKey) {
            ...
            const chosen = document.querySelector('.cards-group .card-wrapper.slot-active') || document.getElementById('hallMainCard');

            // Phase 1 (0ms): single zoom — chosen card scales 1.0→1.45 in one motion
            // (is-zooming wins specificity over is-chosen, so we apply both at once and the small "1.08 wind-up" is bypassed)
        ...
        // ========== Hall of Fame: single-active + right-side tab list ==========
        const HALL_ORDER = ['xiaohai', 'aixleft', 'guo'];
        const HALL_EN = { xiaohai: 'XIAOHAI', aixleft: 'AIXLEFT', guo: 'GUOYONGJUN' };
        let hallActiveChamp = 'xiaohai';

        let isHallSwitching = false;

        // ---------- Hall card stack (one card-wrapper per champion, slot-positioned) ----------
        function buildHallCards() {
            const group = document.querySelector('.cards-group');
            if (!group) return;
            group.innerHTML = '';
            HALL_ORDER.forEach(k => {
                const wrap = document.createElement('div');
                wrap.className = 'card-wrapper';
                wrap.setAttribute('data-champ', k);
                wrap.onclick = () => {
                    if (k === hallActiveChamp) enterDetailFromHall();
                };
                wrap.innerHTML =
                    '<div class="card">' +
        ...
        function updateCardSlots() {
            const idx = HALL_ORDER.indexOf(hallActiveChamp);
            if (idx < 0) return;
            const n = HALL_ORDER.length;
            const prevKey = HALL_ORDER[(idx - 1 + n) % n];
            const nextKey = HALL_ORDER[(idx + 1) % n];
            document.querySelectorAll('.cards-group .card-wrapper').forEach(w => {
                const k = w.getAttribute('data-champ');
                w.classList.remove('slot-prev', 'slot-active', 'slot-next');
                if (k === hallActiveChamp) w.classList.add('slot-active');
                else if (k === prevKey) w.classList.add('slot-prev');
                else if (k === nextKey) w.classList.add('slot-next');
            });
        }

        // ---------- Left-text stack (one inner per champion, slot-positioned) ----------
        function buildHallLeftStack() {
            const stage = document.getElementById('hallLeftStage');
            if (!stage) return;
            stage.innerHTML = '';
            HALL_ORDER.forEach(k => {
                const data = champions[k];
                if (!data) return;
                const human = k === 'xiaohai' ? 'Xiaohai'
                            : k === 'aixleft' ? 'Aixleft'
                            : k === 'guo' ? 'Guo Yongjun' : data.id;
                const inner = document.createElement('div');
                inner.className = 'champ-left-inner';
                inner.setAttribute('data-champ', k);
                inner.innerHTML =
                    '<div class="champ-id">' + human + '</div>' +
                    '<div class="champ-name-hall">' + data.name + '</div>' +
                    '<div class="champ-tag-hall">' + data.tag + '</div>';
                stage.appendChild(inner);
            });
            updateLeftSlots();
        }

        function updateLeftSlots() {
            const idx = HALL_ORDER.indexOf(hallActiveChamp);
            if (idx < 0) return;
            const n = HALL_ORDER.length;
            const prevKey = HALL_ORDER[(idx - 1 + n) % n];
            const nextKey = HALL_ORDER[(idx + 1) % n];
            document.querySelectorAll('#hallLeftStage .champ-left-inner').forEach(node => {
                const k = node.getAttribute('data-champ');
                node.classList.remove('slot-prev', 'slot-active', 'slot-next');
                if (k === hallActiveChamp) node.classList.add('slot-active');
                else if (k === prevKey) node.classList.add('slot-prev');
                else if (k === nextKey) node.classList.add('slot-next');
            });
        }

        function setHallActive(key) {
            if (!champions[key]) return;
            if (key === hallActiveChamp) return;
            if (isHallSwitching) return;
            isHallSwitching = true;

            // ---- All three stacks slide in sync ----
            hallActiveChamp = key;
            updateNavSlots();    // right wheel
            updateCardSlots();   // central card stack
            updateLeftSlots();   // left text stack

            // Brief golden landing glow when the new card settles into the active slot
            const newActiveCard = document.querySelector('.cards-group .card-wrapper[data-champ="' + key + '"]');
            setTimeout(() => {
                if (newActiveCard) newActiveCard.classList.add('is-landing');
            }, 520);
            setTimeout(() => {
                if (newActiveCard) newActiveCard.classList.remove('is-landing');
                isHallSwitching = false;
            }, 900);
        }

        const NAV_ID_LABEL = { xiaohai: 'Xiaohai', aixleft: 'Aixleft', guo: 'Guo Yongjun' };
        const NAV_INITIAL = { xiaohai: 'X', aixleft: 'A', guo: 'G' };

        function renderHallTabs() {
            const stage = document.getElementById('hallNavStage');
            if (!stage) return;
            stage.querySelectorAll('.nav-row').forEach(n => n.remove());
            ...
            HALL_ORDER.forEach(k => {
                ...
                row.setAttribute('data-champ', k);
                const idLabel = NAV_ID_LABEL[k] || data.id || k;
                const initial = NAV_INITIAL[k] || (data.name || k).slice(0, 1);
                row.innerHTML =
                    '<div class="nav-avatar">' + initial + '</div>' +
                    '<div class="nav-name">' +
                        '<span class="nav-id">' + idLabel + '</span>' +
                        '<span class="nav-alias">（' + data.name + '）</span>' +
                    '</div>';
                row.onclick = (e) => {
                    e.stopPropagation();
                    if (k === hallActiveChamp) {
                        enterDetailFromHall();
                    } else {
                        setHallActive(k);
                    }
        ...
        function updateNavSlots() {
            ...
            const prevKey = HALL_ORDER[(idx - 1 + n) % n];
            const nextKey = HALL_ORDER[(idx + 1) % n];
            stage.querySelectorAll('.nav-row').forEach(row => {
                const k = row.getAttribute('data-champ');
                row.classList.remove('slot-prev', 'slot-active', 'slot-next');
                if (k === hallActiveChamp) row.classList.add('slot-active');
                else if (k === prevKey) row.classList.add('slot-prev');
                else if (k === nextKey) row.classList.add('slot-next');
            });
        }

        // Mouse-wheel navigation on the carousel — throttled to feel deliberate
        let navWheelLock = false;
        function bindNavWheel() {
            const stage = document.getElementById('hallNavStage');
            if (!stage || stage._wheelBound) return;
            stage._wheelBound = true;
            stage.addEventListener('wheel', (e) => {
                e.preventDefault();
                if (navWheelLock) return;
                navWheelLock = true;
                setTimeout(() => { navWheelLock = false; }, 420);
                const idx = HALL_ORDER.indexOf(hallActiveChamp);
                if (idx < 0) return;
                const n = HALL_ORDER.length;
                const dir = e.deltaY > 0 ? 1 : -1;
                const nextIdx = (idx + dir + n) % n;
                setHallActive(HALL_ORDER[nextIdx]);
            }, { passive: false });
        }

        function enterDetailFromHall() {
            showDetail(hallActiveChamp);
        }
        ...
        // ========== Soft-focus push transition (game page → champion detail) ==========
        let pushBusy = false;
        function curtainRevealToDetail(detailKey) {
            if (pushBusy) return;
            pushBusy = true;
            const overlay = document.getElementById('pushOverlay');
            const gamePage = document.querySelector('#viewGame .plaza-page');
            if (!overlay || !gamePage) { devGoto('detail', detailKey); pushBusy = false; return; }

            // Phase 1: push the current game page out (scale up + blur + dim)
            //          and fade in a black overlay underneath the motion blur.
            overlay.classList.remove('is-black');
            overlay.classList.add('is-active');
            gamePage.classList.add('push-out');
            requestAnimationFrame(() => overlay.classList.add('is-black'));

            // Phase 2: when fully black, swap to detail view and prep its push-in start state
            setTimeout(() => {
                devGoto('detail', detailKey);
                const detailPage = document.querySelector('#viewDetail .detail-page');
                if (detailPage) {
                    detailPage.classList.add('push-in');
                }
                // Begin fading the black overlay back out as detail emerges from the blur
        ...
        // Initial render of Hall carousel + card stack + left text stack + bind wheel
        try { buildHallCards(); buildHallLeftStack(); renderHallTabs(); bindNavWheel(); } catch(e) {}
        ...
        (function() {
            ...
            // Delegate clicks on video tile to open the modal
